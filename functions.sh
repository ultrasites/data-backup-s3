...
function do_backup() {
  ...
  SOURCE=data_backup_${now}.$EXTENSION
  ...
  # copy volume data to workdir
  cp -R  $BACKUP_DATA $workdir
...
#
# place the backup to s3
#
function save_backup_to_s3() {
  local target=$1
  uri_parser ${target}
  [[ -n "$AWS_ENDPOINT_URL" ]] && AWS_ENDPOINT_OPT="--endpoint-url $AWS_ENDPOINT_URL"
  aws ${AWS_CLI_OPTS} ${AWS_ENDPOINT_OPT} s3 cp ${TMPDIR}/${SOURCE} "${S3_TARGET}/${TARGET}"
  [ $? -ne 0 ] && return 1
  return 0
}
